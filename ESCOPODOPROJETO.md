...
* **Inteligência Articulada:** A CLI utiliza um sistema de **Wizard Interativo** (`questionary`) que guia o usuário no preenchimento de metadados ausentes, oferecendo sugestões inteligentes e garantindo que nenhum dataset seja registrado sem a auditoria completa.
* **Estética e Legibilidade:** Utilização da biblioteca `Rich` para formatação de tabelas e logs no terminal, proporcionando uma experiência de desenvolvedor (DX) moderna.
...
1.  **Dashboard Web (Concluído):** Interface em Streamlit para visualizar e filtrar metadados dos datasets registrados.
2.  **Integridade Automatizada (Concluído):** Comando `verify` para checar sincronia entre Git e Storage SSH.
3.  **Pre-fetch para Treino (Concluído):** Função `prefetch` para download em lote preparando o ambiente para treinos YOLOv11.
4.  **Tags Dinâmicas (Em breve):** Suporte a labels extras (ex: `chuva`, `noite`) via CLI.
5.  **Notificações:** Envio de alertas de novas ingestões via Slack ou Telegram.
